...
func main() {
	...
	for s := 1; s <= 7790; s++ {
		...
		for i := range robots {
			robots[i] = moveRobot(robots[i], mx, my, 1)
			maps[robots[i].cy][robots[i].cx] += 1
		}
		nlx := countLoggestX(maps)
		if nlx > lx {
			iterMaxX = s
			lx = nlx
		}
		nly := countLongestY(maps)
		if nly > ly {
			iterMaxY = s
			ly = nly
		}
	...
	fmt.Println(lx, ly)
	fmt.Println(iterMaxX, iterMaxY)
}

func countLoggestX(maps [][]int) int {
	var l int
...
func countLongestY(maps [][]int) int {
	var l int
	for x := 0; x < len(maps[0]); x++ {
		var c int
		var in bool
		for y := 0; y < len(maps); y++ {
			if in && maps[y][x] == 0 {
