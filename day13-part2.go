...
func costToPrize(machine machine) (int, bool) {
	n := (machine.prize.y*machine.aButton.dx - machine.prize.x*machine.aButton.dy) /
		(machine.bButton.dy*machine.aButton.dx - machine.bButton.dx*machine.aButton.dy)
	m := (machine.prize.x - (n * machine.bButton.dx)) / machine.aButton.dx
	cx := (m * machine.aButton.dx) + (n * machine.bButton.dx)
	cy := (m * machine.aButton.dy) + (n * machine.bButton.dy)
	if cx == machine.prize.x && cy == machine.prize.y {
		cost := (m * machine.aButton.cost) + (n * machine.bButton.cost)
		return cost, true
	}
...
func main() {
	var total int
	machineLines := strings.Split(input, "\n\n")
	for _, machineLine := range machineLines {
		lines := strings.Split(machineLine, "\n")
		aButtonMatches := buttonRegex.FindStringSubmatch(lines[0])
		bButtonMatches := buttonRegex.FindStringSubmatch(lines[1])
		prizeMatches := prizeRegex.FindStringSubmatch(lines[2])

		ax, _ := strconv.Atoi(aButtonMatches[1])
		ay, _ := strconv.Atoi(aButtonMatches[2])
		aButton := button{dx: ax, dy: ay, cost: 3}
		bx, _ := strconv.Atoi(bButtonMatches[1])
		by, _ := strconv.Atoi(bButtonMatches[2])
		bButton := button{dx: bx, dy: by, cost: 1}
		px, _ := strconv.Atoi(prizeMatches[1])
		py, _ := strconv.Atoi(prizeMatches[2])
		prizePos := prize{x: px + 10000000000000, y: py + 10000000000000}
		m := machine{aButton: aButton, bButton: bButton, prize: prizePos}

		cost, ok := costToPrize(m)
		if ok {
			total += cost
		}
	}
