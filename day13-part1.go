...
func costToPrize(m machine) (int, bool) {
	maxList := []int{
		(m.prize.x / m.aButton.dx),
		(m.prize.x / m.bButton.dx),
		(m.prize.y / m.aButton.dy),
		(m.prize.y / m.bButton.dy),
	}
	max := slices.Max(maxList)
	var costList []int
	for i := 0; i <= max; i++ {
		for j := 0; j <= max; j++ {
			cx := (i * m.aButton.dx) + (j * m.bButton.dx)
			cy := (i * m.aButton.dy) + (j * m.bButton.dy)
			if cx > m.prize.x || cy > m.prize.y {
				break
			}
			if cx == m.prize.x && cy == m.prize.y {
				cost := (i * m.aButton.cost) + (j * m.bButton.cost)
